...
???
Upcoming book, very early stages //
GitHub org also has all the materials for this workshop
...
- Navigate to "Tidy Survey Analysis in R" under Projects then click name to get started //
  ...
  - If you want to save your progress, click "Save a Permanent Copy" from within the project //

- We will go through slides of content then work on exercises together in breakout rooms

???
RStudio Cloud is a hosted version of RStudio in the cloud that comes with configuration of the environment for learning, training, teaching //
I will walk through this and suggest to watch as we will have time to set up RStudio Cloud if they haven't already and provide help, don't feel like you have to do it now //
These sessions will not be recorded and we will close RStudio Cloud after the workshop is over but GitHub repo will be available with everything that we cover in this series, such as the code, the slides, etc. // RStudio Cloud is a clone of the GitHub repo so do not have to save anything unless you want to //
...
???
Great package for clean, interesting data for analysis
...
Recommend watching this part because then you will try the rest of the exercises in your breakout rooms
---
...
???
Try to use tidyverse syntax to get warmed up
---
...
- `group_by` with `summarize` does the same thing. The .groups= argument controls the grouping structure of the output. When the output no longer have grouping variables because they are dropped, it becomes ungrouped and a regular tibble.
---
...
- If you want an additional exercise, try it with `group_by` and `summarize`
...
???
- We have subset the columns of this data and created derived variables, code and codebook in repository
...
???
These are the steps in order for survey analysis using sryvr
Today we're going to start off with a survey object and then calculate our point estimates
In the next two sessions, we'll go deeper into filtering and the design object itself
---

## Weighted Analysis for Categorical Variables
...
???
- These functions are what we will go over today
- `survey_count` is new
- has a similar structure as the standard (non-survey) version of count
- `survey_mean` and `survey_prop` calculate means and proportions from complex survey data
- wraps around `svymean` or `svyciprop` from the survey package
...
```
???
- as_survey_design creates a survey object using the sampling design
- need to install github version of survey package if you want CIs with quantiles (which is what we did for the RStudio Cloud account)
- American National Election Studies
- Data provides weights that sum to the sample, but we want to get population estimates
- need to adjust the weight to get it to the population count
- the calculation for the population is available in the repo under DataCleaningScripts
...
```
???
- The srvyr syntax is very similar to standard dplyr syntax; recall beginning of presentation
- switching count() for survey_count()
- we don’t need a wt argument because we defined the weights when we set up the srvyr object.
...
- can change the output count name, `n` is the default, here we call it capital N
...
???
- Using `survey_prop` is equivalent to leaving out the x argument in `survey_mean`
- using `proportion = TRUE` make confidence intervals more accurate near 0 and 1
- `prop_method` says the type of proportion method to use if `proportion` is TRUE
- `proportion`/ `prop_method` impact the CI only
- `deff` defines whether the design effect should be returned
- `df` defines the degrees of freedom for the confidence interval `var_type`
...
```
???
- We will talk more about this next time but you can use `filter` after creating design object to subset data, in this case, removing `NAs`
...
```
???
- We add an interaction for the groups with `interact`
- This outputs the joint distribution
- It groups by multiple variables as if they were a single variable which allows for calculating proportions that sum to 100% across more than a single grouping variable
...
```
???
- The design effect is a ratio of two variances.
- Use `deff=TRUE` option in the `survey_mean` function
- It is a logical value to indicate whether the design effect should be returned.
---
...
???
- We can also calculate confidence intervals within summarize and using `survey_prop` or `survey_mean`
...
```
???
- going to switch to the survey package (no more dplyr syntax for functions but can still pipe our data)
- when we want to test categorical distributions we use `svychisq`
- it takes a formula, and the survey design data to create contigency tables
- default statistic is the Rao-Scott second-order correction
- only works for two-way tables
---
...
- We want to know if there's a statistically significant relationship
- This low p-value tells us a relationship exists between the two values
---
...
- The other type of test is a Wald test based on the differences between the observed cell counts and those expected under independence
- Used by SUDAAN
...
???
- Before we jump into modeling, let's refresh our formula notation
- These are how formulas are represented in R
---
...
???
- `svyglm` gives us survey-weighted generalised linear models.
- notice how you remove `NA`
...
???
- Estimates show our associations between our predictors and our outcome
