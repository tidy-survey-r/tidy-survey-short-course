...
???
Walk through this and suggest to watch as we will have time to set up RStudio Cloud if they haven't already and provide help
...
- `group_by` with `summarise` does the same thing. The .groups= argument controls the grouping structure of the output. When the output no longer have grouping variables because they are dropped, it becomes ungrouped and a regular tibble.
---
...
- If you want an additional exercise, try it with `group_by` and `summarise`
...
- `survey_mean` calculates means and proportions from complex survey data
